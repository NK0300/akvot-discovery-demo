...
סטטוס: **v1.2.2** (v1.1.1 אושר; §11 = ביקורת מלאה + אכיפה) · **CONTRACT ONLY** (אין קוד בקומיט הזה) · מימוש: שרת, על branch מעל `48d09a6`.
...
**v1.2.2: שער commit יחיד (מחייב).**
- **שער אחד.** `mayCommitDossier` הוא השער היחיד לכל dossier או QID: `wiki_seeded`, `latin_evidence`, knownIdentities, התאמת כותרת, והחזרה מהמטמון (כולל רשומות ישנות, שנבדקות מחדש בזמן הקריאה ולא נמחקות).
- **סדר קבוע בתוך השער:**
  1. `ceiling === 'clear'`
  2. אין נעילה של שם נפוץ (Smith-class, או שם עברי נפוץ בלי seed או focus)
  3. ראיה
- **stamp.** השער מחזיר אובייקט מוטבע ב־WeakSet פרטי, וה־builder של uiState=dossier זורק שגיאה בלי stamp.
- **הסתייגות דיוק.** `recordHas` הוא תנאי הכרחי, לא מספיק. הוא לא מבטל נעילה. `latin_evidence` נשאר רק לשמות Latin שאינם common. Smith+IBM מול רשומה עם affiliation IBM, בכותרת exact או בשדה מובנה, מחזיר candidates בלי QID (M02).
- **הבהרת Chief לשלב 3.** `recordHas` נדרש רק כשה־ctx הוא מה שמשחרר את ה־commit, כלומר כשבלי ctx לא היה dossier. זהות שעומדת על הראיות שלה בלי ctx (Assaf, Q47507930) נשארת dossier גם כש־ctx לא נמצא ברשומה. ctx שלא נמצא הוא UNKNOWN ולא סתירה. הוא מוצג כ־`providedContext`, בלי `match:`.
- **recall נשמר.** R01: Assaf עם IBM ובלי ctx. R02: שם Latin לא נפוץ עם כותרת exact ו־affiliation מובנית.
- **citation (A3).** מותר רק ל־`fetchId` של רשומה שנשלפה בריצה הזו, ולא לפי השוואת מחרוזת URL. הרשומה חייבת להכיל גם את key של השם וגם את ערך הטענה. אחרת הטענה היא UNKNOWN.
- **A7.** ערך של 3 תווים או פחות, וכל country או city, נחשב match רק מול `STRUCTURED_FIELDS` (רשימה אחת ב־`ctxEvidence.js`, משותפת ל־`whyFor` ולבונוס הציון).
- **שחזור `ds1.`.** 0 קריאות ל־providers בכל מקרה:
  - `400 invalid_session`
  - `422 empty_seed`
