...
ok('cache-bust c1m6', /discovery-ui\.js\?v=c1m6/.test(html));
...
// §25 client — explicit seedKind in request body + Registry displayLabel w/ identity guard (?v=c1m4)
{
  ...
  ok('§26) ודאי dropped (server-list union)', A.labelFamily('wikidata', null, { wikidata: { he: 'מקור ודאי', en: 'Source' } }) === 'ויקינתונים');
  ok('§26) clean "Rosa Mendes" kept (no collapse w/o hidden chars)', A.safeDisplayLabel({ he: 'x', en: 'Rosa Mendes' }, 'en') === 'Rosa Mendes');
  ok('§26) "Ro\u200Bsa Me" still kept unless it forms a deny word', A.safeDisplayLabel({ he: 'x', en: 'Ro\u200Bsa' }, 'en') === 'Ro sa');
