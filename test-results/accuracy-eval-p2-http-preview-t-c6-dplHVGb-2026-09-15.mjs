#!/usr/bin/env node
/**
 * Accuracy (דיוק) HTTP Acc — P2 Preview T-C6 John Rappaport (dpl_HVGb)
 * Preview: https://akvot-simple-demo-29yid8jo9-k-akvot.vercel.app
 * Deploy: dpl_HVGb86Qrxe9Ztte3ESMCc7F3tnHk
 * REJECTED: dpl_FEog… — do NOT use
 * Case set: handoff/P2-CASES-דיוק-2026-09-15.json (N=18) — EXPECTED unchanged
 * Access: vercel curl (Deployment Protection; plain curl → 302)
 * CRITICAL: A06 not dossier · qid null · not Q47507930/Q105094696
 * CRITICAL: S03 POST nested {q, ctx:{org,city,country:US}} · not dossier · not Q1701775
 * CRITICAL: Assaf A01/A03 dossier Q47507930 · pw=0 for Acc GO
 * POST nested body {q, ctx:{…}} for ctx cases (NOT flat spread)
 * NO product code changes · NO EXPECTED rewrite · NO promote · HOLD promote
 */
import fs from 'fs';
...
const PREVIEW_URL = (process.env.AKVOT_BASE || process.env.BASE || process.env.ORIGIN || 'https://akvot-simple-demo-29yid8jo9-k-akvot.vercel.app').replace(/\/$/, '');
const ORIGIN = 'https://akvot-simple-demo.vercel.app';
const TIMEOUT_MS = 65000;
const CONCURRENCY = Number(process.env.AKVOT_CONCURRENCY || 2);
const DEPLOY = 'dpl_HVGb86Qrxe9Ztte3ESMCc7F3tnHk';
const PHASE_EXPECTED = 'orchestrator-v0-b';
const CASE_SET = path.join(__dirname, 'handoff', 'P2-CASES-דיוק-2026-09-15.json');
const OUT_MD = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-T-C6-דיוק-2026-09-15.md');
const OUT_JSON = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-T-C6-דיוק-2026-09-15.json');
const OUT_REPORT = path.join(__dirname, 'ACCURACY_REPORT-P2-HTTP-Preview-T-C6-dplHVGb-2026-09-15.md');
const OUT_LOG = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-T-C6-דיוק-2026-09-15.run.log');
const OUT_SMOKE = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-T-C6-דיוק-2026-09-15.smoke.md');
const OUT_SMOKE_JSON = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-T-C6-דיוק-2026-09-15.smoke.json');
const OUT_CONSOLE = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-T-C6-דיוק-2026-09-15.console.log');
...
async function callLookup({ method = 'GET', q, ctx = {} }) {
  let snap;
  if (method === 'POST') {
    // MUST nested {q, ctx:{…}} — flat spread was the pw bug on Acc HTTP
    const body = { q, nocache: 1 };
    if (ctx && typeof ctx === 'object' && Object.keys(ctx).length) body.ctx = ctx;
...
function judge(c, snap) {
  ...
  if (mustNot.includes('dossier') && ui === 'dossier') {
    ...
    // T-C6 / A06: dossier alone is pretty-wrong even without faces
    if (faces || c.id === 'P2-A06' || isSafetyCase(c)) prettyWrong = true;
  }
  ...
  // T-C6 / P2-A06: bare John Rappaport — not dossier, qid must be null, not Assaf/wikiExact bleed
  if (c.id === 'P2-A06') {
    if (ui === 'dossier') {
      if (!fails.some((f) => f.includes('must_not dossier'))) fails.push('must_not dossier (T-C6)');
      prettyWrong = true;
    }
    if (qid != null) {
      fails.push(`qid must be null (got ${qid})`);
      if (ui === 'dossier' || qid === 'Q47507930' || qid === 'Q105094696') prettyWrong = true;
    }
    if (qid === 'Q105094696' || (raw && /"Q105094696"/.test(raw) && ui === 'dossier')) {
      if (!fails.some((f) => f.includes('Q105094696'))) fails.push('must_not qid:Q105094696');
      prettyWrong = true;
    }
  }
...
async function smokePostS03x3() {
  const body = { q: 'John Smith', ctx: { org: 'IBM', city: 'New York', country: 'US' }, nocache: 1 };
  const runs = [];
  for (let i = 1; i <= 3; i++) {
    log(`[smoke] → POST nested S03 run ${i}`);
    const snap = await vercelCurl('/api/lookup?nocache=1', { method: 'POST', body });
    let data = null;
    try { data = JSON.parse(snap.body || ''); } catch {}
    const ui = data?.uiState || null;
    const qid = data?.qid || null;
    const faces = hasFaces(data);
    const fails = [];
    let prettyWrong = false;
    if (snap.err) fails.push(`network: ${snap.err}`);
    else if (snap.status !== 200) fails.push(`http ${snap.status}`);
    else {
      if (ui === 'dossier') { fails.push('must_not dossier'); prettyWrong = !!faces || true; }
      if (qid === 'Q1701775') { fails.push('must_not Q1701775'); prettyWrong = true; }
      if (faces) { fails.push('faces must be 0'); prettyWrong = true; }
      if (!['candidates', 'thin', 'need_context'].includes(ui)) fails.push(`uiState=${ui}`);
    }
    const row = {
      run: i, pass: fails.length === 0, prettyWrong, fails,
      http: snap.status, ms: snap.ms, ui, qid, faces, photo: !!data?.photo,
      requestId: data?.requestId || parseHeader(snap.headers, 'x-request-id'),
      body,
    };
    log(`[smoke] ← run ${i} ${row.pass ? 'PASS' : 'FAIL'} ui=${ui} qid=${qid||'-'} faces=${faces} ms=${snap.ms}`);
    runs.push(row);
    await new Promise((r) => setTimeout(r, 800));
  }
  return {
    case: 'P2-S03 POST nested ×3 smoke',
    body,
    runs,
    allPass: runs.every((r) => r.pass),
    prettyWrong: runs.filter((r) => r.prettyWrong).length,
  };
}
...
async function main() {
  ...
  if (DEPLOY.startsWith('dpl_FEog') || String(PREVIEW_URL).includes('REJECTED')) {
    log('FATAL: rejected deployment — abort');
    process.exit(2);
  }
  ...
  // Extra A06 T-C6 guard: qid null · not dossier · not Q105094696/Q47507930
  if (a06) {
    const q = a06.actual.qid;
    const ui = a06.actual.uiState;
    const extra = [];
    if (ui === 'dossier') { extra.push('must_not dossier (T-C6)'); a06.prettyWrong = true; }
    if (q != null) {
      extra.push(`qid must be null (got ${q})`);
      if (ui === 'dossier' || q === 'Q47507930' || q === 'Q105094696') a06.prettyWrong = true;
    }
    if (q === 'Q105094696' || q === 'Q47507930') {
      extra.push(`must_not qid:${q}`);
      a06.prettyWrong = true;
    }
    if (extra.length) {
      a06.pass = false;
      a06.status = 'FAIL';
      a06.fails = [...(a06.fails || []), ...extra];
      a06.errorType = a06.prettyWrong ? 'PRETTY-WRONG' : 'FP';
    }
  }
  ...
  const smoke = await smokePostS03x3();
  log(`SMOKE POST×3 allPass=${smoke.allPass} pw=${smoke.prettyWrong}`);

  // Build MD
  let md = '';
  md += `# ACCURACY_EVAL · P2 HTTP · Preview · T-C6 · dpl_HVGb86Qrxe9Ztte3ESMCc7F3tnHk · דיוק · 2026-09-15\n\n`;
  ...
  md += `**Rules:** vercel curl · GET simple q · **POST nested** {q, ctx:{…}} for ctx · Origin=${ORIGIN} · nocache=1 · timeout=65s · concurrency=${CONCURRENCY}  \n`;
  md += `**Constraints:** NO product code changes · NO EXPECTED rewrite · NO promote · REJECTED dpl_FEog…  \n`;
  md += `**Focus:** T-C6 A06 John Rappaport · S03 POST nested · Assaf A01/A03  \n`;
  ...
  md += `| P2-A06 T-C6 John Rappaport | ${a06 ? `**${a06.status}** ui=${a06.actual.uiState} qid=${a06.actual.qid || 'null'} faces=${a06.actual.faces} pw=${a06.prettyWrong}` : '—'} |\n`;
  ...
  md += `- **A06** John Rappaport (T-C6) → ${a06?.status} · ui=${a06?.actual.uiState} · qid=${a06?.actual.qid ?? 'null'} · faces=${a06?.actual.faces} (must_not dossier · Q47507930 · Q105094696 · qid null)\n`;
  md += `- **L03** latency Assaf → ${l03?.status} · wall_ms=${l03?.actual.ms} · timings.total=${l03?.actual.timings_total}\n\n`;

  md += `## Optional POST×3 smoke · P2-S03 nested same shape\n\n`;
  md += `| Run | PASS/FAIL | ui | qid | faces | ms | fails |\n|-----|-----------|----|-----|-------|----|-------|\n`;
  for (const r of smoke.runs) {
    md += `| ${r.run} | **${r.pass ? 'PASS' : 'FAIL'}** | ${r.ui} | ${r.qid || '-'} | ${r.faces} | ${r.ms} | ${r.fails.length ? r.fails.join('; ') : '—'} |\n`;
  }
  md += `\n**Smoke:** allPass=${smoke.allPass} · prettyWrong=${smoke.prettyWrong} · body=\`${JSON.stringify(smoke.body)}\`\n\n`;

  md += `## Hebrew summary (room)\n\n`;
  const heSummary = [
    `בדיקת HTTP Acc POST-CTX מקונן ל־P2 Preview (\`${DEPLOY}\`): N=${N}, PASS=${PASS}, FAIL=${FAIL}, pretty-wrong=${prettyWrong}, SAFETY=${safetyLock}, S03=${s03?.status}/${s03?.actual.uiState}, smoke×3=${smoke.allPass}.`,
  ...
  const jsonOut = {
    meta: {
      title: 'ACCURACY_EVAL-P2-HTTP-Preview-T-C6-dpl_HVGb86Qrxe9Ztte3ESMCc7F3tnHk',
      date: '2026-09-15',
      ...
      access: 'vercel curl',
      post_body_shape: 'nested {q, ctx:{…}} for ctx cases',
  ...
  let smokeMd = `# ACCURACY_EVAL · POST×3 smoke · P2-S03 · dpl_HVGb86Qrxe9Ztte3ESMCc7F3tnHk · דיוק · 2026-09-15\n\n`;
  smokeMd += `**Deploy:** \`${DEPLOY}\` · Preview ${PREVIEW_URL}\n`;
  smokeMd += `**Body:** \`${JSON.stringify(smoke.body)}\`\n`;
  smokeMd += `**allPass:** ${smoke.allPass} · **prettyWrong:** ${smoke.prettyWrong}\n\n`;
  smokeMd += `| Run | Result | ui | qid | faces | ms |\n|-----|--------|----|-----|-------|----|\n`;
  for (const r of smoke.runs) smokeMd += `| ${r.run} | ${r.pass ? 'PASS' : 'FAIL'} | ${r.ui} | ${r.qid || '-'} | ${r.faces} | ${r.ms} |\n`;
  fs.writeFileSync(OUT_SMOKE, smokeMd, 'utf8');
  fs.writeFileSync(OUT_SMOKE_JSON, JSON.stringify(smoke, null, 2), 'utf8');

  // Update ACCURACY_REPORT.md briefly as latest P2 Preview
  let report = `# ACCURACY_REPORT\n\n`;
  report += `**Latest eval:** ACCURACY_EVAL-P2-HTTP-Preview-T-C6-דיוק-2026-09-15 · deploy \`dpl_HVGb86Qrxe9Ztte3ESMCc7F3tnHk\`  \n`;
  ...
  report += `\n## Prior eval\n`;
  report += `Prior: POST-CTX dpl_2Qrf · LOCAL T-C6 layer PASS · REJECTED dpl_FEog… — **not** this dpl. This report is HTTP Acc T-C6 Preview on \`${DEPLOY}\` only. HOLD promote.\n`;
  fs.writeFileSync(OUT_REPORT, report, 'utf8');
  fs.writeFileSync(OUT_LOG, logLines.join('\n') + '\n', 'utf8');
  fs.writeFileSync(OUT_CONSOLE, logLines.join('\n') + '\n', 'utf8');
