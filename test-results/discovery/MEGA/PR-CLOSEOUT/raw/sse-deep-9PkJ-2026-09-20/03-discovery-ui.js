...
(function () {
  ...
  function renderProgressStrip() {
    ...
    const stClass = st === 'reconnecting' ? ' reconnecting' : st === 'failed_soft' ? ' failed' : st === 'complete' ? ' complete' : '';
    const reconnectHint =
    ...
    const cursorHint =
      discState.cursor != null
        ? `<span class="disc-source-tag">cursor:${esc(discState.cursor)}</span>`
        : '';
      ...
      total
        ...
        : st === 'complete'
          ...
          : st === 'reconnecting'
            ...
            : 18;
    return `
      <div class="disc-progress" role="status" aria-live="polite" data-disc-status="${esc(st)}">
    ...
    out.innerHTML = `
      ...
      <div class="disc-layout">
        ...
        <div class="disc-feed" aria-label="ממצאים">
          ...
          ${cards || (discState.status === 'running' || discState.status === 'partial' || discState.status === 'reconnecting'
