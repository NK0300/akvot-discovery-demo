#!/usr/bin/env node
/**
 * MEGA Acc SHORT smoke on CANONICAL Preview dpl_9PkJ… + Core P0 · דיוק · 2026-09-20
 * Targets: dpl_9PkJ… (supersedes CAVh) + alias dpl_8ag… LOCKED. HOLD promote. NO Core rewrite.
...
const RAW = join(OUT_DIR, 'raw/acc-post-promote');
const CORE_RAW = join(OUT_DIR, 'raw/acc-post-promote/core');
mkdirSync(RAW, { recursive: true });
mkdirSync(CORE_RAW, { recursive: true });

/** POST-PROMOTE Acc · Discovery alias PROMOTED · Core LOCKED · NO further promote */
const DISC_BASE = 'https://akvot-discovery.vercel.app';
const DISC_BASE_2 = 'https://akvot-discovery-k-akvot.vercel.app';
...
const OUT_DISC_JSON = join(OUT_DIR, 'ACC-POST-PROMOTE-דיוק-2026-09-20.json');
const OUT_DISC_MD = join(OUT_DIR, 'ACC-POST-PROMOTE-דיוק-2026-09-20.md');
const OUT_CORE_JSON = join(OUT_DIR, 'ACC-POST-PROMOTE-CORE-דיוק-2026-09-20.json');
const OUT_CORE_MD = join(OUT_DIR, 'ACC-POST-PROMOTE-CORE-דיוק-2026-09-20.md');
...
console.log(`[acc-post-promote] start ${started}`);
console.log(`[acc-post-promote] Discovery alias PROMOTED ${DISC_DPL} · Core LOCKED ${ALIAS_DPL} · NO further promote`);
...
const coreNotAvyhr = aliasHealth.json?.build === ALIAS_DPL && aliasHealth.json?.build !== DISC_DPL;
console.log('[health] Core build still 8ag (not Avyhr)?', coreNotAvyhr, aliasHealth.json?.build);
...
const seeds = [
  ...
  {
    label: 'disc-seed-en-person',
    seed: 'Jane Doe',
    hints: { locale: 'en' },
    note: 'EN person soft — entity-agnostic',
  },
  {
    label: 'disc-seed-smith-ctx',
    seed: 'John Smith',
    hints: { org: 'IBM', city: 'New York', country: 'US' },
    note: 'Smith+IBM/NY/US adversarial — NEVER Q1701775',
  },
  {
  ...
  {
    label: 'disc-seed-org-company',
    seed: 'Microsoft',
    hints: { kind: 'org' },
    note: 'Org/company — entity-agnostic no false dossier',
  },
];
...
const accDiscGo = discPass && discLeak === 0 && discHealthOk && seedsPosted >= 5 && dossierBindings.length === 0 && b23ScrubPass && coreNotAvyhr;
const coreAliasGo = corePass && coreLeak === 0 && corePw === 0 && aliasHealthOk;

const finished = nowJerusalem();

const discResult = {
  doc: 'ACC-POST-PROMOTE-דיוק-2026-09-20',
  checked: started,
  finished,
  ...
  promote: false,
  hold_promote: true,
  no_further_promote: true,
  phase: 'POST-PROMOTE Acc verification only',
  discovery_alias: DISC_BASE,
  discovery_alias_2: DISC_BASE_2,
  discovery_dpl: DISC_DPL,
  alias: ALIAS_BASE,
  alias_dpl: ALIAS_DPL,
  core_build_still_8ag: coreNotAvyhr,
  core_build_observed: aliasHealth.json?.build ?? null,
  forbidden: 'Q1701775',
...
const coreResult = {
  doc: 'ACC-POST-PROMOTE-CORE-דיוק-2026-09-20',
  checked: started,
  finished,
  alias: ALIAS_BASE,
  dpl: ALIAS_DPL,
...
const discMd = `# ACC POST-PROMOTE · דיוק · 2026-09-20
...
**Discovery alias (PROMOTED):** \`${DISC_BASE}\` · also \`${DISC_BASE_2}\` → \`${DISC_DPL}\`  
**Core production (LOCKED):** \`${ALIAS_BASE}\` → \`${ALIAS_DPL}\` (must prove unchanged · not Avyhr)  
...
| Gate | Result | Detail |
|------|--------|--------|
| **Acc Discovery alias** | **${discResult.verdict === 'GO' ? 'PASS' : 'FAIL'}** | Seeds ${seedsPosted}/${seeds.length} · leak=${discLeak} · dossier bindings=${dossierBindings.length} |
| B23 contradictions scrub | **${b23ScrubPass ? 'PASS' : 'FAIL'}** | Smith POST+GET findingIds · full-body Q1701775=0 |
| Adversarial Smith+IBM/NY/US | **${smithPost && smithGet && smithPost.leakage_count===0 && smithGet.leakage_count===0 ? 'PASS' : 'FAIL'}** | POST+GET HIT full-body scrub |
...
| Core build still 8ag | **${yn(coreNotAvyhr)}** | observed \`${aliasHealth.json?.build ?? 'null'}\` · not Avyhr |
| **Core Acc P0** | **${coreAliasGo ? 'PASS' : 'FAIL'}** | Assaf/כהן/Smith · pw=${corePw} · leak=${coreLeak} |
...
|--------|---------------|-------|
| Discovery \`/api/health\` | \`${discHealth.json?.build ?? 'null'}\` · store=\`${storeBackend}\` | ${yn(discHealthOk)} |
| Core alias \`/api/health\` | \`${aliasHealth.json?.build ?? 'null'}\` | ${yn(aliasHealthOk)} · **still 8ag=${yn(coreNotAvyhr)}** |
...
|----------|--------|--------|
| health.build still \`dpl_8ag…\` | **${yn(coreNotAvyhr)}** | observed \`${aliasHealth.json?.build ?? 'null'}\` |
| Assaf → Q47507930 | **${yn(assaf.pass)}** | ui=\`${assaf.ui}\` · qid=\`${assaf.qid}\` · faces=${assaf.faces} |
| כהן soft | **${yn(cohen.pass)}** | ui=\`${cohen.ui}\` · faces=${cohen.faces} |
| Smith POST+ctx nocache | **${yn(smithCold.pass)}** | ui=\`${smithCold.ui}\` · qid=\`${smithCold.qid ?? 'None'}\` · faces=${smithCold.faces} · never Q1701775 |
| Smith POST+ctx warm | **${yn(smithWarm.pass)}** | ui=\`${smithWarm.ui}\` · qid=\`${smithWarm.qid ?? 'None'}\` · faces=${smithWarm.faces} · never Q1701775 |
| Acc leakage / pw | **${yn(coreLeak === 0 && corePw === 0)}** | leak=${coreLeak} · pw=${corePw} |

**Core P0:** **${coreAliasGo ? 'PASS' : 'FAIL'}** · build \`${aliasHealth.json?.build ?? 'null'}\` · pw=${corePw} · leakage=${coreLeak}
...
## Decision
- Acc Discovery alias: **${discResult.verdict === 'GO' ? 'PASS' : 'FAIL'}** · leak=${discLeak} · Seeds ${seedsPosted} · B23 **${b23ScrubPass ? 'PASS' : 'FAIL'}**
- Core P0: **${coreAliasGo ? 'PASS' : 'FAIL'}** · build \`${aliasHealth.json?.build ?? 'null'}\` · pw=${corePw} · leakage=${coreLeak}
- storeBackend observed: **${storeBackend ?? 'UNKNOWN'}**
- **NO promote** · **HOLD further promote** · Core alias \`${ALIAS_DPL}\` remains **LOCKED**
`;

const coreMd = `# ACC POST-PROMOTE · Core Acc P0 · דיוק · 2026-09-20

**STATUS:** **${coreResult.verdict}** · POST-PROMOTE prove Core unchanged · **NO further promote · NO Core rewrite**  
**Checked:** ${started} → ${finished} (Asia/Jerusalem, UTC+3)  
**Alias:** \`${ALIAS_BASE}\`  
**Expect / observed build:** \`${ALIAS_DPL}\` / \`${aliasHealth.json?.build ?? 'null'}\` · still 8ag=${yn(coreNotAvyhr)}  
**Access:** \`vercel curl --deployment ${ALIAS_DPL} --scope k-akvot\`  
**Denylist:** \`forbiddenIdentitiesVersion\` **${FIV_EXPECTED}**  
**Paired Discovery alias (PROMOTED):** \`${DISC_DPL}\` (Avyhr)
...
## Decision
- Core Acc P0 alias: **${coreResult.verdict}** · build still \`${ALIAS_DPL}\`
- **NO promote** · **HOLD further promote** · alias \`${ALIAS_DPL}\` remains **LOCKED**
`;
