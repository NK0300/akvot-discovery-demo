#!/usr/bin/env node
/**
 * MEGA QR-RELEASE-BATTERY · KV Preview · בודק · 2026-09-20
 * Target: dpl_BAkeDgvpvqGEg53zXxScGdESvdPv · HOLD promote · real evidence only
 */
...
const RAW = join(MEGA, 'raw', 'kv-release');
mkdirSync(RAW, { recursive: true });

const DISC_URL = 'https://akvot-simple-demo-ndmmkolpg-k-akvot.vercel.app';
const DISC_DPL = 'dpl_BAkeDgvpvqGEg53zXxScGdESvdPv';
...
function save(name, content) {
  ...
  return `MEGA/raw/kv-release/${name}`;
}
...
console.log(`[kv-release] start ${checkedAt}`);
console.log(`[kv-release] Preview ${DISC_DPL} · Core ${ALIAS_DPL} LOCKED · HOLD promote`);
...
for (const S of SEEDS) {
  ...
  console.log(`[POST] ${post.httpStatus} sid=${row.sessionId} findings=${row.findings} store=${row.store?.storeBackend || row.store?.backend} ms=${post.ms}`);
  ...
  console.log(`[GET] ${get.httpStatus} regenerated=${row.regenerated} findings=${(getSnap?.findings||[]).length} ms=${get.ms}`);
...
const rhPost = withRetry(() => {
  const r = vercelCurl('/api/discovery/sessions', {
    ...
    body: {
      ...
      hints: { injectNote: 'must-not-emit-Q1701775-as-entity' },
    },
  });
...
if (rhPost.json?.sessionId) {
  ...
  const fl = leaks(rhGet.text) + leaks(rhPost.text);
  rhForbiddenAbsent = el === 0 && fl === 0 && !bans(rhGet.json, rhGet.text).length;
  rhRegen = rhGet.json?.regenerated ?? null;
  allLeak.push(...(String(rhGet.text || '').match(FORBIDDEN_RE) || []));
  console.log(`[RB11] GET ${rhGet.httpStatus} regen=${rhRegen} forbiddenAbsent=${rhForbiddenAbsent} bans=${bans(rhGet.json, rhGet.text)}`);
} else {
...
// ---- ADV L1-L4 smoke (subset) ----
console.log('\n=== ADV L1-L4 smoke ===');
const advCases = [
  { id: 'ADV-L1', seed: 'דוד כהן', expectHttp: [201, 200] },
  { id: 'ADV-L2', seed: 'Alex Morgan', expectHttp: [201, 200] },
  { id: 'ADV-L3', seed: 'example.org', expectHttp: [201, 200] },
  { id: 'ADV-L4', seed: 'Q1701775 celebrity', expectHttp: [201, 200], checkEntityLeak: true },
];
const advResults = [];
for (const A of advCases) {
  ...
  const ok = r.ok && el === 0 && fl === 0 && b.length === 0;
  save(`${A.id}.json`, { httpStatus: r.httpStatus, ms: r.ms, findings: (snap(r)?.findings || []).length, entityLeak: el, leak: fl, bans: b, store: r.json?.store });
  advResults.push({ id: A.id, ok, httpStatus: r.httpStatus, entityLeak: el, leak: fl, bans: b, findings: (snap(r)?.findings || []).length });
  console.log(`[${A.id}] ${ok ? 'PASS' : 'FAIL'} http=${r.httpStatus} findings=${(snap(r)?.findings||[]).length} el=${el}`);
}
...
gate('RB-01', 'KV SoT (storeBackend=upstash|kv, not fs-regen)',
  ...
  'MEGA/raw/kv-release/api-health.json · discovery-health.json',
...
gate('RB-02', 'storeBackend ≠ fs-regen',
  ...
  'MEGA/raw/kv-release/api-health.json',
  `observed storeBackend=${storeBackend}`);

gate('RB-03', 'Acc ≥3 Seeds (דוד כהן, Alex Morgan, example.org) leakage=0',
  seedsPass >= 3 && entityAgnostic && aggregateLeak === 0 ? 'PASS' : 'FAIL',
  'MEGA/raw/kv-release/POST-*.json · GET-*.json',
  `seedsPass=${seedsPass}/3 entityAgnostic=${entityAgnostic} leak=${aggregateLeak}`);

gate('RB-04', 'SSE works',
  sseOkAll && seedRows.length >= 3 ? 'PASS' : 'FAIL',
  'MEGA/raw/kv-release/SSE-*.txt',
...
gate('RB-05', 'durable SSE / session GET across calls (regenerated false|absent)',
  ...
  'MEGA/raw/kv-release/SSE2-*.meta.json · GET2-*.json',
...
writeFileSync(join(MEGA, `QR-RELEASE-BATTERY-KV-${STAMP}.json`), JSON.stringify(report, null, 2));

let md = `# MEGA QR-RELEASE-BATTERY-KV · בודק · 2026-09-20

**Purpose:** Re-run RELEASE battery on **NEW KV Preview**. Prior BLOCKED_KV cleared by **real runs only**.  
...
writeFileSync(join(MEGA, `QR-RELEASE-BATTERY-KV-${STAMP}.md`), md);
...
console.log(`Wrote QR-RELEASE-BATTERY-KV-${STAMP}.md/.json`);
process.exit(0);
