#!/usr/bin/env node
/**
 * MEGA Acc-DISC KV Preview + Core P0 KV Gate · דיוק · 2026-09-20
 * Targets: dpl_BAke… (KV LIVE) + alias dpl_8ag… LOCKED. HOLD promote. NO Core rewrite.
...
const RAW = join(__dirname, 'raw/acc-disc-kv');
const CORE_RAW = join(__dirname, 'raw/core-p0-kv-gate');
mkdirSync(RAW, { recursive: true });
mkdirSync(CORE_RAW, { recursive: true });

const DISC_BASE = 'https://akvot-simple-demo-ndmmkolpg-k-akvot.vercel.app';
...
const OUT_DISC_JSON = join(__dirname, 'MEGA-ACC-DISC-KV-דיוק-2026-09-20.json');
const OUT_DISC_MD = join(__dirname, 'MEGA-ACC-DISC-KV-דיוק-2026-09-20.md');
const OUT_CORE_JSON = join(__dirname, 'MEGA-ACC-CORE-P0-KV-GATE-דיוק-2026-09-20.json');
const OUT_CORE_MD = join(__dirname, 'MEGA-ACC-CORE-P0-KV-GATE-דיוק-2026-09-20.md');
...
console.log(`[acc-disc-kv] start ${started}`);
console.log(`[acc-disc-kv] preview ${DISC_DPL} · alias ${ALIAS_DPL} · HOLD promote`);
...
const accDiscGo = discPass && discLeak === 0 && discHealthOk && seedsPosted >= 3 && dossierBindings.length === 0;
const coreAliasGo = corePass && coreLeak === 0 && corePw === 0 && aliasHealthOk;

const finished = nowJerusalem();

const discResult = {
  doc: 'MEGA-ACC-DISC-KV-דיוק-2026-09-20',
  checked: started,
  finished,
...
const coreResult = {
  doc: 'MEGA-ACC-CORE-P0-KV-GATE-דיוק-2026-09-20',
  checked: started,
  finished,
  alias: ALIAS_BASE,
  dpl: ALIAS_DPL,
...
const discMd = `# MEGA · Acc-DISC KV Preview · דיוק · 2026-09-20

**Checked:** ${started} → ${finished} (Asia/Jerusalem, UTC+3)  
**Promote:** **HOLD** (explicit · no promote · no Core rewrite)  
**Discovery Preview (KV LIVE):** \`${DISC_BASE}\` · \`${DISC_DPL}\`  
...
const coreMd = `# MEGA · Core Acc P0 KV Gate · דיוק · 2026-09-20
...
**Paired Acc-DISC Preview:** \`${DISC_DPL}\` (KV)
