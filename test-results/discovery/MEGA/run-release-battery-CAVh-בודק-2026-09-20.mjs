 ...
 * Target: dpl_CAVh2rDSNDFfjrGzw8EJWJNmPN4e · after B23 scrub + KV REST refresh · HOLD promote
 */
...
const RAW = join(MEGA, 'raw', 'release-cavh');
mkdirSync(RAW, { recursive: true });

const DISC_URL = 'https://akvot-simple-demo-p68nhr48g-k-akvot.vercel.app';
...
function save(name, content) {
  ...
  return `MEGA/raw/release-cavh/${name}`;
}
...
console.log(`[release-cavh] start ${checkedAt}`);
console.log(`[release-cavh] Preview ${DISC_DPL} · Core ${ALIAS_DPL} LOCKED · HOLD promote`);
...
gate('RB-01', 'KV SoT (storeBackend=upstash|kv, not fs-regen)',
  ...
  'MEGA/raw/release-cavh/api-health.json · discovery-health.json',
...
gate('RB-02', 'storeBackend ≠ fs-regen',
  ...
  'MEGA/raw/release-cavh/api-health.json',
...
gate('RB-03', 'Acc ≥3 Seeds + Smith+ctx (IBM/NY/US) leakage=0 incl contradictions',
  ...
  'MEGA/raw/release-cavh/POST-*.json · GET-*.json · SMITH-CTX-SCAN.json',
...
gate('RB-05', 'durable SSE / session GET across calls (regenerated false|absent)',
  ...
  'MEGA/raw/release-cavh/SSE2-*.meta.json · GET2-*.json',
...
gate('RB-08', 'leakage=0 deep scan all surfaces including contradictions',
  ...
  'MEGA/raw/release-cavh/ · SMITH-CTX-SCAN.json',
...
gate('RB-11', 'rehydrate scrub (GET session after POST — forbidden absent)',
  ...
  'MEGA/raw/release-cavh/RB11-*.json',
...
gate('RB-12', 'ADV subset L1-L4 smoke PASS',
  advPass ? 'PASS' : 'FAIL',
  'MEGA/raw/release-cavh/ADV-L*.json',
...
writeFileSync(join(MEGA, `QR-RELEASE-BATTERY-CAVh-${STAMP}.json`), JSON.stringify(report, null, 2));

let md = `# MEGA QR-RELEASE-BATTERY-CAVh · בודק · 2026-09-20
...
- \`QR-RELEASE-BATTERY-CAVh-${STAMP}.json\`
...
writeFileSync(join(MEGA, `QR-RELEASE-BATTERY-CAVh-${STAMP}.md`), md);
...
console.log(`Wrote QR-RELEASE-BATTERY-CAVh-${STAMP}.md/.json`);
process.exit(0);
