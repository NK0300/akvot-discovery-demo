#!/usr/bin/env node
/**
 * Acc PRE-PROMOTE VERIFY — stricter than FIX-3 · Preview only · NO promote
...
const SCOPE = 'k-akvot';
const RAW = '/workspace/akvot-quick-demo/test-results/wp3/ACC-P0-PRE-PROMOTE-VERIFY-raw';
const OUT_MD = '/workspace/akvot-quick-demo/test-results/wp3/ACC-P0-PRE-PROMOTE-VERIFY-דיוק-2026-09-20.md';
const OUT_JSON = '/workspace/akvot-quick-demo/test-results/wp3/ACC-P0-PRE-PROMOTE-VERIFY-דיוק-2026-09-20.json';
...
function analyze(label, r) {
  ...
  const candLeak = (r.json?.candidates || []).some(c => FORBIDDEN.test(String(c.id || '')) || FORBIDDEN.test(String(c.qid || '')) || FORBIDDEN.test(JSON.stringify(c)));
  ...
  const rankedLeak = (r.json?.ranked || r.json?.sourcesRanked || []).some(s => FORBIDDEN.test(JSON.stringify(s)));
  const facetsLeak = r.json?.facets ? FORBIDDEN.test(JSON.stringify(r.json.facets)) : false;
  const hasLeak = leaks.length > 0 || candLeak || dossierLeak || sourcesLeak || rankedLeak || facetsLeak;
  return {
    label,
    ms: r.ms,
    status: r.status,
    ...
    candLeak, dossierLeak, sourcesLeak, rankedLeak, facetsLeak,
    ...sum,
  };
}
...
const results = {
  meta: {
    title: 'ACC-P0-PRE-PROMOTE-VERIFY',
    role: 'דיוק',
    date: '2026-09-20',
    ...
    stricter_than: 'FIX-3 + Acc Gate FORMAL',
    battery: [
...
results.gate = {
  ...
  promote: false,
  alias_frozen: true,
  Acc_PRE_PROMOTE: (smithOk && assafOk && cohenOk && tc6Ok && scanOk && results.leakage_total === 0 && facesSmithOk)
    ? 'GO'
    : 'NO-GO',
  Chief_REVIEW: 'Alias frozen until Chief/שרת promote · דיוק does NOT promote',
};
...
function pubRows(cell) {
  return (cell?.rows || []).map(r => ({
    ...
    rankedLeak: r.rankedLeak,
    facetsLeak: r.facetsLeak,
...
const md = `# Acc PRE-PROMOTE VERIFY · דיוק · 2026-09-20

**STATUS:** Acc PRE-PROMOTE **${g.Acc_PRE_PROMOTE}** · **NO promote by דיוק** · alias **FROZEN**  
...
| **pw** | **${results.pw}** | |
| **Acc PRE-PROMOTE** | **${g.Acc_PRE_PROMOTE}** | Chief/שרת promote only · alias frozen |
...
## Handoff
**Acc PRE-PROMOTE ${g.Acc_PRE_PROMOTE}** → Chief/שרת promote only · **HOLD** · alias \`${ALIAS_FROZEN}\` frozen · Preview \`${DPL}\` only.
`;

writeFileSync(OUT_MD, md);
console.log(JSON.stringify(results.gate, null, 2));
process.stderr.write(`\n=== Acc PRE-PROMOTE ${results.gate.Acc_PRE_PROMOTE} leakage=${results.leakage_total} pw=${results.pw} ===\n`);
