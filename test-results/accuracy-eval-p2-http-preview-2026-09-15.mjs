#!/usr/bin/env node
/**
 * Accuracy (דיוק) HTTP Acc — P2 RC Preview
 * Preview: https://akvot-simple-demo-48x7gx9a6-k-akvot.vercel.app
...
const PREVIEW_URL = (process.env.AKVOT_BASE || 'https://akvot-simple-demo-48x7gx9a6-k-akvot.vercel.app').replace(/\/$/, '');
const ORIGIN = 'https://akvot-simple-demo.vercel.app';
const TIMEOUT_MS = 65000;
const CONCURRENCY = Number(process.env.AKVOT_CONCURRENCY || 2);
const DEPLOY = 'dpl_DqzucMKykHVssELsCi91waZ5LCmg';
const PHASE_EXPECTED = 'orchestrator-v0-b';
const CASE_SET = path.join(__dirname, 'handoff', 'P2-CASES-דיוק-2026-09-15.json');
const OUT_MD = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-דיוק-2026-09-15.md');
const OUT_JSON = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-דיוק-2026-09-15.json');
const OUT_REPORT = path.join(__dirname, 'ACCURACY_REPORT.md');
const OUT_LOG = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-דיוק-2026-09-15.run.log');
...
async function main() {
  ...
  // Build MD
  let md = '';
  md += `# ACCURACY_EVAL · P2 HTTP · Preview · דיוק · 2026-09-15\n\n`;
  ...
  const jsonOut = {
    meta: {
      title: 'ACCURACY_EVAL-P2-HTTP-Preview',
      date: '2026-09-15',
  ...
  report += `**Latest eval:** ACCURACY_EVAL-P2-HTTP-Preview-דיוק-2026-09-15  \n`;
