#!/usr/bin/env node
/**
 * Accuracy (דיוק) HTTP Acc — P2 Preview POST-CTX nested (dpl_2Qrf)
 * Preview: https://akvot-simple-demo-8t702yisg-k-akvot.vercel.app
 ...
 * CRITICAL: POST nested body {q, ctx:{org,city,country}} for ctx cases (NOT flat spread)
...
const PREVIEW_URL = (process.env.AKVOT_BASE || process.env.BASE || process.env.ORIGIN || 'https://akvot-simple-demo-8t702yisg-k-akvot.vercel.app').replace(/\/$/, '');
const ORIGIN = 'https://akvot-simple-demo.vercel.app';
const TIMEOUT_MS = 65000;
const CONCURRENCY = Number(process.env.AKVOT_CONCURRENCY || 2);
const DEPLOY = 'dpl_2QrfuXhQg1Jt9q2NBdedkKJHwTqT';
const PHASE_EXPECTED = 'orchestrator-v0-b';
const CASE_SET = path.join(__dirname, 'handoff', 'P2-CASES-דיוק-2026-09-15.json');
const OUT_MD = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-POST-CTX-דיוק-2026-09-15.md');
const OUT_JSON = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-POST-CTX-דיוק-2026-09-15.json');
const OUT_REPORT = path.join(__dirname, 'ACCURACY_REPORT-P2-HTTP-Preview-POST-CTX-dpl2Qrf-2026-09-15.md');
const OUT_LOG = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-POST-CTX-דיוק-2026-09-15.run.log');
const OUT_SMOKE = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-POST-CTX-דיוק-2026-09-15.smoke.md');
const OUT_SMOKE_JSON = path.join(__dirname, 'ACCURACY_EVAL-P2-HTTP-Preview-POST-CTX-דיוק-2026-09-15.smoke.json');
...
async function main() {
  ...
  // Build MD
  let md = '';
  md += `# ACCURACY_EVAL · P2 HTTP · Preview · POST-CTX nested · dpl_2QrfuXhQg1Jt9q2NBdedkKJHwTqT · דיוק · 2026-09-15\n\n`;
  ...
  const jsonOut = {
    meta: {
      title: 'ACCURACY_EVAL-P2-HTTP-Preview-POST-CTX-dpl_2QrfuXhQg1Jt9q2NBdedkKJHwTqT',
      date: '2026-09-15',
  ...
  let smokeMd = `# ACCURACY_EVAL · POST×3 smoke · P2-S03 · dpl_2QrfuXhQg1Jt9q2NBdedkKJHwTqT · דיוק · 2026-09-15\n\n`;
  ...
  report += `**Latest eval:** ACCURACY_EVAL-P2-HTTP-Preview-POST-CTX-דיוק-2026-09-15 · deploy \`dpl_2QrfuXhQg1Jt9q2NBdedkKJHwTqT\`  \n`;
  ...
  report += `\n## Prior eval\n`;
  report += `Prior Preview Acc: SMITH-CTX dpl_9nM3 (flat POST bug · S03 pw) and Preview dpl_Dqzuc — **not** this dpl. This report is POST nested Acc on \`${DEPLOY}\` only. Local POST-CTX: ACCURACY_REEVAL-P2-LOCAL-POST-CTX-דיוק-2026-09-15.\n`;
